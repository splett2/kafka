...
class DynamicConfigChangeTest extends KafkaServerTestHarness {
  ...
  def testIpQuotaInitialization(): Unit = {
    ...
    val quotaManagers = server.quotaManagers
    assertEquals(10L, quotaManagers.connections.connectionRateForIp(InetAddress.getByName("1.2.3.4")))
    assertEquals(20L, quotaManagers.connections.connectionRateForIp(InetAddress.getByName("2.4.6.8")))
  }
  ...
  def testIpQuotaConfigChange(): Unit = {
    ...
    val quotaManagers = servers.head.quotaManagers

    TestUtils.retry(10000) {
      val overrideIpQuota = quotaManagers.connections.connectionRateForIp(overriddenIp)
      val defaultIpQuota = quotaManagers.connections.connectionRateForIp(defaultQuotaIp)
    ...
    TestUtils.retry(10000) {
      val defaultIpQuota = quotaManagers.connections.connectionRateForIp(overriddenIp)
    ...
    TestUtils.retry(10000) {
      val defaultIpQuota = quotaManagers.connections.connectionRateForIp(overriddenIp)
      assertEquals(s"IP $overriddenIp should have reset IP quota to ${Long.MaxValue}", Long.MaxValue, defaultIpQuota)
    }
  }
