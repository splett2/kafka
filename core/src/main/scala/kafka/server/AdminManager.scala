...
class AdminManager(val config: KafkaConfig,
  ...
  def describeClientQuotas(filter: ClientQuotaFilter): Map[ClientQuotaEntity, Map[String, Double]] = {
    ...
    if (!filter.strict)
      handleDescribeClientQuotas(userComponent, clientIdComponent, filter.strict) ++ handleDescribeIpQuotas(ipComponent)
    else if (userComponent.isDefined || clientIdComponent.isDefined)
      handleDescribeClientQuotas(userComponent, clientIdComponent, filter.strict)
    else if (ipComponent.isDefined)
      handleDescribeIpQuotas(ipComponent)
    else
      Map.empty
  }
  ...
  def handleDescribeIpQuotas(ipComponent: Option[ClientQuotaFilterComponent]): Map[ClientQuotaEntity, Map[String, Double]] = {
